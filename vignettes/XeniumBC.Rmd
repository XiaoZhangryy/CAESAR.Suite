...
This vignette introduces the CAESAR workflow for the analysis of Xenium BC spatial transcriptomics dataset. In this vignette, the workflow of CAESAR consists of five steps
...
We demonstrate the application of CAESAR to Xenium data. In this vignette, the input data includes: `BC_scRNAList` — two scRNA-seq reference datasets, each with 3,000 cells; `BC_XeniumList` — two Xenium target datasets, each with 3,000 cells; and `BC_feature_imgList` — two feature matrices containing histology image information from the Xenium target datasets. For more details with respect to histology image feature extraction, see xxx. The genes of scRNA-seq reference datasets and Xenium target datasets are aligned. Those data can be downloaded and load to the current working path by the following command:
```{r}
githubURL <- "https://github.com/XiaoZhangryy/CAESAR/blob/master/vignettes_data/BC_scRNAList.rda?raw=true"
download.file(githubURL, "BC_scRNAList.rda", mode='wb')
BC_scRNAList <- readRDS("./BC_scRNAList.RDS")

print(BC_scRNAList)

githubURL <- "https://github.com/XiaoZhangryy/CAESAR/blob/master/vignettes_data/BC_XeniumList.rda?raw=true"
download.file(githubURL, "BC_XeniumList.rda", mode='wb')
BC_XeniumList <- readRDS("./BC_XeniumList.RDS")

print(BC_XeniumList)

githubURL <- "https://github.com/XiaoZhangryy/CAESAR/blob/master/vignettes_data/BC_feature_imgList.rda?raw=true"
download.file(githubURL, "BC_feature_imgList.rda", mode='wb')
BC_feature_imgList <- readRDS("./BC_feature_imgList.RDS")
...
library(CAESAR) # load the package of CAESAR method
library(Seurat)
library(ProFAST)
library(ggplot2)
```
